...
### 1.1 2026-08-26 一致性修复范围

本轮在既有流程上补齐七项启动前阻断：A 的最小范数选择、B 与 A 的确定性差异、cycle 122 失败停止、物理失败不得切换 backup、容量倍率精确冻结、检查点完整验证，以及移除未批准的 cycle 25 SOH 下限。同步补齐运行上下文/源码指纹、标定与留出目标隔离、初始化恢复、候选/排名/报告审计和相应测试。不得启动长循环。

首次一致性修复验证：`156 passed in 23.65s`；第 17 节启动前修复完成后最终验证：`171 passed in 23.79s`。未执行正式拟合或真实 cycle 0–30。
...
## 17. 启动前最小风险修复实施顺序（2026-08-26）

本节落实设计规范第 16 节。实施期间不运行真实 PyBaMM 长循环，也不单独执行真实 cycle 0–30；只运行单元测试、fake runner/backend 测试和不求解的静态门禁。

### 17.1 统一检查点续算入口

1. 删除 Stage-1 编排器直接反序列化检查点、回填检查点自身指纹并提前跳过阶段的逻辑；
2. 所有已有检查点均调用 `W10Runner.run(..., resume_checkpoint=...)`；
3. `W10Runner` 在重新计算当前输入、UDDS、初态、环境、有效参数和运行上下文指纹，并完成输出回滚校验后，判断检查点是否已达到请求停止 cycle；
4. 已达到停止点时，仅在 RPT、提交清单和协议阶段均完整的条件下返回 `PAUSED`，不得执行下一循环；
5. 用 fake backend/monkeypatch 测试覆盖有效空操作和各类指纹/提交不匹配拒绝。

### 17.2 门禁、候选和失败审计

1. 保留独立 baseline cycle 0–30 回归门禁；正式拟合必须先通过该门禁，失败即停止；
2. 中/高倍率探针继续执行独立 cycle 25 安全门；
3. 探针代表按“改善基线误差、不过冲、实验距离、远离上界、重试、ID”稳定排序；
4. 将 `NUMERICALLY_CENSORED` 与 `PHYSICALLY_INFEASIBLE` 分别写入候选清单、组合运行结果、排名和最终报告；
5. 为组合 A 增加求解后条件检查，保证最小范数解仍位于允许的预测 RMSE 等效带内。

### 17.3 进度和报告工件

1. `stage1_progress.json` 增加三个倍率、最新可用 SOH 残差和预计完成时间；
2. 同步追加 `stage1_progress_history.jsonl`，只记录和估算时间，不改变流程；
3. 从最终验证候选的 `degradation_summary.csv` 提取 total SEI、可逆析锂、死锂、总析锂、负极 LAM 和正极 LAM；
4. 生成 `stage1_soh_residuals.png` 与 `stage1_mechanism_trends.png`，并把 CSV/图片路径写入最终报告；
5. 未完成验证时明确把机理趋势标记为待生成，不伪造空趋势。

### 17.4 完成与启动结论

1. 运行定向测试和全量快速测试；
2. 审计正式输入、冻结容量倍率、CLI 命令和输出目录状态；
3. 结论分为“代码与静态门禁可启动”和“真实数值续算等价性待正式拟合首个 baseline cycle 0–30 门禁验证”；
4. 不替用户启动正式拟合。
